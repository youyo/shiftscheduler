...
func GetShifts(c *gin.Context) (int, *Shifts, error) {
	LoggerDebug(c, "Called models.GetShifts")
	...
	var shifts Shifts
	q := fmt.Sprintf("select * from %s", ShiftsTable)
	if _, err := sess.SelectBySql(q).Load(&shifts); err != nil {
		LoggerError(c, fmt.Sprintf("failed to query. query: %s", q))
		return 400, nil, err
	}

	return 200, &shifts, nil
}

func CreateShift(c *gin.Context, name, userUuid string) (int, string, error) {
	LoggerDebug(c, "Called models.CreateShift")
	...
	q := fmt.Sprintf("insert into %s (uuid,name,user_uuid) values (?,?,?)", ShiftsTable)
	if _, err := tx.InsertBySql(q, uuid, name, userUuid).Exec(); err != nil {
		tx.Rollback()
		LoggerError(c, fmt.Sprintf("failed to query. query: %s, uuid: %s, name: %s, user_uuid: %s", q, uuid, name, userUuid))
		return 400, "", err
	}
...
func GetShift(c *gin.Context, uuid string) (int, *Shift, error) {
	LoggerDebug(c, "Called models.GetShift")
	...
	var shift Shift
	q := fmt.Sprintf("select * from %s where uuid=?", ShiftsTable)
	if _, err := sess.SelectBySql(q, uuid).Load(&shift); err != nil {
		LoggerError(c, fmt.Sprintf("failed to query. query: %s, uuid: %s", q, uuid))
		return 400, nil, err
	}

	return 200, &shift, nil
}

func DeleteShift(c *gin.Context, uuid string) (int, string, error) {
	LoggerDebug(c, "Called models.DeleteShift")
	...
	q := fmt.Sprintf("delete from %s where uuid=?", ShiftsTable)
...
func PatchShift(c *gin.Context, uuid string, shiftDetail ShiftDetail) (int, string, error) {
	LoggerDebug(c, "Called models.PatchShift")
	...
	if _, err := tx.Update(ShiftsTable).SetMap(m).Where("uuid=?", uuid).Exec(); err != nil {
		tx.Rollback()
		LoggerError(c, fmt.Sprintf("failed to query. map: %v, uuid: %s", m, uuid))
		return 400, "", err
	}

	tx.Commit()

	return 200, "updated", nil
}
